...
const app = new Elysia()
  ...
  .get('/limit', ({ set }) => {
    ...
    try {
      ...
      const storage$ = forkJoin([computeQuota$, storageUsage$]).pipe(
        map(([quotaRes, usageRes]) => ({ name: 'storage', usage: usageRes.data, limit: quotaRes.data.maxStorage })),
        catchError((err) => {
          return of();
        })
      );
      const bandwidth$ = forkJoin([computeQuota$, bandwidthUsage$]).pipe(
